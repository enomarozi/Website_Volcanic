...
class MeteorologyService:
    ...
    def load(self):
        ...
        with Dataset(self.file_path, mode="r") as ds:
            ...
            if missing:
                raise ValueError(
                    f"Required meteorological variables are missing: "
                    f"{', '.join(missing)}"
                )
    ...
    def nearest_grid(self, latitude: float, longitude: float):
        ...
        return {
            ...
            "latitude_index": int(lat_index),
            "longitude_index": int(lon_index)
        }

    def pressure_to_pa(self, pressure_hpa):
        return np.asarray(
            pressure_hpa,
            dtype=float
        ) * 100.0

    def potential_temperature(self, temperature, pressure_hpa):
        ...
        return temperature * (p0 / pressure_hpa) ** exponent

    def geopotential_to_altitude(self, geopotential):
        geopotential = np.asarray(
            geopotential,
            dtype=float
        )
    ...
    def atmospheric_profile(
        ...
        latitudes = data["latitude"]
        longitudes = data["longitude"]
        pressure_levels = data["pressure_level"]
        ...
        if time_index < 0 or time_index >= len(data["time"]):
            raise IndexError(
                "Invalid time_index."
            )
        ...
        altitude = self.geopotential_to_altitude(
            geopotential
        )
